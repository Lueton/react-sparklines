---
title: SparklinesBar
description: SparklinesBar API reference
---
...
- **Optional**
- **Type:** `ReactElement<SVGElement> | ((props: BarProps) => ReactElement<SVGElement>) | BarProps | boolean`
...
- **Optional**
- **Type:** `Omit<PresentationAttributesWithProps<SVGPathElement>, "points" | "name" | "radius" | "width" | "height"> & {radius?: SparklinesRadius}`
...
If true, the chart will automatically fill the full width of its container using a `ResizeObserver`. The chart re-renders when the container is resized. Height remains fixed. When `responsive` is enabled, the `width` prop is used as a fallback for SSR or environments without `ResizeObserver`.
